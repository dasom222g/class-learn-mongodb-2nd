...
// 2. filter: 배열안의 요소들 중에서 특정 조건을 만족하는 요소들로 새로운 배열을 리턴
...
const isSomeName = names.some((name) => name.length >= 4); // true
console.log("🚀 ~ isSomeName:", isSomeName);
const isEveryName = names.every((name) => name.length >= 4); // false
